<template>
  <div>
    ...
    <template v-if="layout === 'gundam'">
      ...
      <div class="an-page-label">▶ MODULE · ACTUALITÉS / JOURNAL DE BORD</div>
      ...
      <div class="max-w-6xl mx-auto px-6 pt-6 pb-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
          <div>
            <div class="an-eyebrow">
              <span class="an-eyebrow-line"></span>
              // ACCUEIL · ACTUALITÉS
            </div>
            <h1 class="text-[22px] font-black uppercase tracking-wide text-white leading-tight">Actualités</h1>
            <p v-if="!loading" class="text-[9px] font-mono text-ink-3 mt-1 tracking-widest">
              {{ filtered.length }} ARTICLE{{ filtered.length !== 1 ? 'S' : '' }} DISPONIBLES
            </p>
          </div>
          <!-- Recherche -->
          <div class="relative w-full sm:w-60">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-orange/50 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input v-model="search" type="text" placeholder="Rechercher…"
              class="w-full bg-bg-1 border border-orange/20 pl-9 pr-4 py-2 text-[11px] font-mono text-white placeholder:text-ink-3 outline-none focus:border-orange/50 transition-colors" />
          </div>
        </div>
        ...
        <div class="flex gap-1 flex-wrap mb-5">
          <button
            ...
            class="text-[9px] font-mono tracking-widest uppercase px-3 py-1.5 border transition-colors"
            :class="activeCategory === cat.id
              ? 'border-orange bg-orange/10 text-orange'
              : 'border-orange/15 text-ink-3 hover:border-orange/35 hover:text-ink-1'"
          >{{ cat.label }}</button>
        </div>

        <!-- Loading -->
        <div v-if="loading" class="flex flex-col gap-2">
          <div v-for="i in 6" :key="i" class="h-16 bg-bg-1 border border-orange/10 border-l-[3px] border-l-orange/20 animate-pulse"></div>
        </div>

        <!-- Aucun résultat -->
        <div v-else-if="!filtered.length" class="text-center py-20 text-[11px] font-mono text-ink-3 tracking-widest">
        ...
        <div v-else class="flex flex-col">
          <RouterLink
            ...
            class="flex items-start gap-3 bg-bg-1/60 px-3 py-3 transition-all border border-orange/[0.1] border-l-[3px] border-l-orange/20 hover:bg-orange/[0.05] hover:border-orange/25 hover:border-l-orange"
          >
            <!-- Miniature -->
            <div class="w-24 h-14 shrink-0 relative overflow-hidden border-l-2 border-orange/30">
              <img loading="lazy" v-if="item.thumb" :src="item.thumb" class="absolute inset-0 w-full h-full object-cover opacity-80" />
              <div class="absolute inset-0" :style="{ background: item.gradient || '#1a1a2a', opacity: item.thumb ? 0.45 : 1 }"></div>
              <div v-if="!item.thumb" class="absolute inset-0 flex items-center justify-center text-3xl opacity-25">{{ item.icon }}</div>
            </div>
            <!-- Contenu -->
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <span class="badge" :class="getCategoryBadge(item.category)">{{ item.category }}</span>
                <span class="text-[9px] font-mono text-ink-3">{{ formatDate(item.createdAt) }}</span>
                <span v-if="item.views" class="text-[9px] font-mono text-ink-3 ml-auto">{{ fmt(item.views) }} vues</span>
              </div>
        ...
        <!-- Charger plus -->
        <div v-if="!loading && filtered.length > visibleCount" class="flex justify-center mt-6">
          <button @click="visibleCount += 12" class="btn-outline text-[10px] font-mono tracking-widest uppercase px-6 py-2">
            // CHARGER PLUS
            <span class="text-ink-3 ml-2">({{ filtered.length - visibleCount }})</span>
          </button>
        </div>
      ...
      <div class="max-w-6xl mx-auto px-6 py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p v-if="!loading" class="text-[12px] text-ink-3 mt-0.5">{{ filtered.length }} article{{ filtered.length !== 1 ? 's' : '' }}</p>
          </div>
        ...
        <!-- Aucun résultat -->
        <div v-else-if="!filtered.length" class="text-center py-20 text-ink-3 text-[13px]">Aucun article trouvé</div>
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <RouterLink
            v-for="item in visible" :key="item._id"
            :to="`/actualite/${item._id}`"
            class="group bg-bg-1 border border-white/[0.06] border-l-[3px] border-l-white/10 rounded-2xl overflow-hidden hover:border-white/15 hover:border-l-orange hover:-translate-y-0.5 transition-all flex flex-col"
          >
            <div class="relative h-36 shrink-0 overflow-hidden">
              ...
              <div v-if="!item.thumb" class="absolute inset-0 flex items-center justify-center text-[56px] opacity-20 select-none">{{ item.icon }}</div>
            ...
            <div class="flex flex-col flex-1 p-4">
              ...
              <h2 class="text-[13px] font-bold text-white leading-snug mb-2 group-hover:text-orange transition-colors line-clamp-2">{{ item.title }}</h2>
        ...
        <div v-if="!loading && filtered.length > visibleCount" class="flex justify-center mt-8">
          <button @click="visibleCount += 12" class="btn-outline px-6 py-2 text-[12px]">
            Charger plus
...
.an-eyebrow-line { display: block; width: 20px; height: 1px; background: rgb(var(--color-orange)); flex-shrink: 0; }
</style>
